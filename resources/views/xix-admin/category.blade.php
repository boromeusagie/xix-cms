      ...
      <div class="card-body">
        <form action="{{ route('admin-category-store') }}" method="post">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control @error('categoryName') is-invalid @enderror" name="categoryName" id="categoryName" placeholder="Category Name">
            @error('categoryName')
        ...
        <form name="updateItem" method="post">
          <div class="modal-body">
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control @error('categoryName') is-invalid @enderror" value="{{ old('categoryName') }}" name="categoryName" id="categoryName" placeholder="Category Name">
                  @error('categoryName')
                    <div class="alert alert-danger">{{ $message }}</div>
                  @enderror
              </div>
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                  <textarea name="description" class="form-control" id="description" cols="3" rows="3" placeholder="(optional)">{{ old('categoryName') }}</textarea>
              </div>
            </div>
          </div>
...
@section('scripts')
    <script>
      $('#updateItem').on('show.bs.modal', function (event) {
        ...
        let url = "{{ route('admin-category-update', ['id' => ':id']) }}"
        url = url.replace(':id', idItem)
        
        var modal = $(this)
        modal.find('input#categoryName').val(name)
      ...
      $('#deleteItem').on('show.bs.modal', function (event) {
        ...
        var name = button.data('name')
        var idItem = button.data('id')
        let url = "{{ route('admin-category-destroy', ['id' => ':id']) }}"
        url = url.replace(':id', idItem)
        
        var modal = $(this)
        modal.find('.modal-text').text('Are you sure delete category "' + name + '"?')
