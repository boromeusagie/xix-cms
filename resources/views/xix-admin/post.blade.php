        ...
        <tbody>
          @foreach ($pages as $index => $item)
              <tr>
                ...
                <td>
                    <div class="btn-group">
                        <a href="{{ route('admin-post-show', ['id' => $item->id]) }}" class="btn btn-sm text-primary" title="Edit">
...
@section('scripts')
    <script>
      $('#deleteItem').on('show.bs.modal', function (event) {
        ...
        let url = "{{ route('admin-post-destroy', ['id' => ':id']) }}"
        url = url.replace(':id', idItem)
        
        var modal = $(this)
        modal.find('.modal-text').text('Are you sure delete post "' + name + '"?')
