...
@section('content')
<form action="{{ route('admin-page-update', ['id' => $page->id]) }}" method="post" name="addPage" id="addPageForm">
    @csrf
    <div class="row">
        <div class="col-lg-9">
            <div class="card">
                <div class="card-body">
                    <div class="form-group">
                        <input type="text" name="title" class="form-control form-control-lg  @error('title') is-invalid @enderror" placeholder="Enter Page Title" value="{{ $page->title }}">
                        @error('title')
                    ...
                    <div class="form-group">
                        <textarea name="content" class="textarea @error('content') is-invalid @enderror" placeholder="Write an awesome content here...."
                          style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; padding: 10px;">{{ $page->content }}</textarea>
                          @error('content')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
